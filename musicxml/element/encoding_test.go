...
func TestEncoding_UnmarshalXML(t *testing.T) {
	...
	tests := []struct {
      ...
      <supports element="stem" type="yes"/>
				...
				start: xml.StartElement{Name: xml.Name{Local: "encodin"}, Attr: []xml.Attr{}},
			},
			wantErr: true,
			wantObj: Encoding{},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(*eO, tt.wantObj); diff != "" {
				t.Errorf("Encoding.UnmarshalXML() value is mismatch (-eO, +wantObj):%s\n", diff)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(ox, tt.wantXml); diff != "" {
				t.Errorf("Encoding.MarshalXML() value is mismatch (-ox, +wantXml):%s\n", diff)
			}
		})
	}
}
