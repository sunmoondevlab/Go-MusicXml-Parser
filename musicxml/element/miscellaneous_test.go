	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(*mO, tt.wantObj); diff != "" {
				t.Errorf("Miscellaneous.UnmarshalXML() value is mismatch (-mO, +wantObj):%s\n", diff)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(ox, tt.wantXml); diff != "" {
				t.Errorf("Miscellaneous.MarshalXML() value is mismatch (-ox, +wantXml):%s\n", diff)
			}
		})
	}
}
